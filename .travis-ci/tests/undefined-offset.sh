...
##
#
tests__undefined_offset() {
    printf 'Started: %s\n' "${FUNCNAME[0]}"

    local -a source_list=( first second third )
    print_array source_list

    local -a deleted_expected
    local -a deleted_list
    local -a expected_list=( first third )

    local -a test_one=( "${source_list[@]}" )
    array_splice --target test_one --deleted deleted_list --element second
    ...
    array_splice --target test_two --deleted deleted_list --index 1
